import { useState, useEffect } from 'preact/hooks';
import QRCode from 'qrcode';
import { useAuth } from '../hooks/useAuth';
import { detectPlatform, type Platform } from '../utils/platform';

export function LoginPage() {
  const { signIn } = useAuth();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [platform, setPlatform] = useState<Platform>('desktop');
  const [qrDataUrl, setQrDataUrl] = useState<string | null>(null);

  useEffect(() => {
    const p = detectPlatform();
    setPlatform(p);
    if (p === 'desktop') {
      QRCode.toDataURL(window.location.origin, { width: 240, margin: 1 })
        .then(setQrDataUrl)
        .catch(() => setQrDataUrl(null));
    }
  }, []);

  const handleSignIn = async () => {
    setLoading(true);
    setError(null);
    try {
      await signIn();
    } catch (err: any) {
      setError(err.message || 'ログインに失敗しました');
    } finally {
      setLoading(false);
    }
  };

  const isDesktop = platform === 'desktop';

  return (
    <div class="flex flex-col items-center justify-center" style={{
      minHeight: '100vh',
      padding: 'var(--spacing-lg)',
      background: 'linear-gradient(180deg, var(--color-primary) 0%, #2D5A6B 100%)'
    }}>
      <div class="card text-center" style={{
        width: '100%',
        maxWidth: '400px',
        padding: 'var(--spacing-xl)'
      }}>
        <div style={{ fontSize: '64px', marginBottom: 'var(--spacing-md)' }}>💊</div>
        <h1 style={{
          fontSize: 'var(--font-size-2xl)',
          fontWeight: 600,
          marginBottom: 'var(--spacing-sm)',
          color: 'var(--color-gray-900)'
        }}>
          おくすりリマインダー
        </h1>
        <p style={{
          color: 'var(--color-gray-600)',
          marginBottom: 'var(--spacing-xl)'
        }}>
          毎日の薬の飲み忘れを防ぎます
        </p>

        {isDesktop ? (
          <div style={{ marginBottom: 'var(--spacing-md)' }}>
            <p style={{
              fontSize: 'var(--font-size-lg)',
              fontWeight: 600,
              color: 'var(--color-gray-900)',
              marginBottom: 'var(--spacing-xs)'
            }}>
              📱 スマホで開いて使ってね
            </p>
            <p style={{
              fontSize: 'var(--font-size-sm)',
              color: 'var(--color-gray-600)',
              marginBottom: 'var(--spacing-lg)',
              lineHeight: 1.6
            }}>
              QR コードをスマホで読み取って開いてください
            </p>

            <div style={{
              display: 'flex',
              justifyContent: 'center',
              marginBottom: 'var(--spacing-lg)'
            }}>
              {qrDataUrl ? (
                <img
                  src={qrDataUrl}
                  alt="このアプリを開く QR コード"
                  width={220}
                  height={220}
                  style={{
                    background: 'var(--color-white)',
                    padding: 'var(--spacing-sm)',
                    borderRadius: 'var(--radius)',
                    boxShadow: '0 4px 16px rgba(45, 90, 107, 0.12), 0 1px 3px rgba(45, 90, 107, 0.08)'
                  }}
                />
              ) : (
                <div style={{ width: '220px', height: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div class="spinner" />
                </div>
              )}
            </div>

            <div style={{
              display: 'flex',
              flexDirection: 'column',
              gap: 'var(--spacing-xs)',
              marginBottom: 'var(--spacing-md)',
              alignItems: 'center'
            }}>
              <div style={{
                display: 'inline-flex',
                alignItems: 'center',
                padding: '6px 14px',
                background: '#F0F7FA',
                color: '#2D5A6B',
                borderRadius: '999px',
                fontSize: 'var(--font-size-sm)',
                fontWeight: 500
              }}>
                iPhone は <strong style={{ marginLeft: '4px' }}>Safari</strong>
              </div>
              <div style={{
                display: 'inline-flex',
                alignItems: 'center',
                padding: '6px 14px',
                background: '#F1F8E9',
                color: '#33691E',
                borderRadius: '999px',
                fontSize: 'var(--font-size-sm)',
                fontWeight: 500
              }}>
                Android は <strong style={{ marginLeft: '4px' }}>Chrome</strong>
              </div>
            </div>

            <p style={{
              fontSize: 'var(--font-size-xs)',
              color: 'var(--color-gray-600)'
            }}>
              ※ PC では通知機能をご利用いただけません
            </p>
          </div>
        ) : (
          <>
            {error && (
              <div style={{
                background: '#fef2f2',
                color: 'var(--color-danger)',
                padding: 'var(--spacing-md)',
                borderRadius: 'var(--radius)',
                marginBottom: 'var(--spacing-md)',
                fontSize: 'var(--font-size-sm)'
              }}>
                {error}
              </div>
            )}

            <button
              onClick={handleSignIn}
              disabled={loading}
              class="btn btn-block"
              style={{
                background: 'var(--color-white)',
                border: '1px solid var(--color-gray-300)',
                padding: 'var(--spacing-md)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                gap: 'var(--spacing-sm)'
              }}
            >
              {loading ? (
                <div class="spinner" style={{ width: '20px', height: '20px' }} />
              ) : (
                <>
                  <svg width="20" height="20" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                  </svg>
                  <span>Googleでログイン</span>
                </>
              )}
            </button>
          </>
        )}

        <div style={{
          marginTop: 'var(--spacing-xl)',
          padding: 'var(--spacing-md)',
          background: 'var(--color-gray-100)',
          borderRadius: 'var(--radius)',
          fontSize: 'var(--font-size-sm)',
          color: 'var(--color-gray-600)'
        }}>
          <h3 style={{ fontWeight: 600, marginBottom: 'var(--spacing-sm)' }}>
            このアプリでできること
          </h3>
          <ul style={{
            textAlign: 'left',
            listStyle: 'none',
            display: 'flex',
            flexDirection: 'column',
            gap: 'var(--spacing-xs)',
            width: 'fit-content',
            margin: '0 auto'
          }}>
            <li>✓ 複数の薬を登録・管理</li>
            <li>✓ 服用時刻に通知でリマインド</li>
            <li>✓ 飲み忘れ時は自動で再通知</li>
            <li>✓ 服用履歴をカレンダーで確認</li>
          </ul>
        </div>

        {!isDesktop && (
          <p style={{
            marginTop: 'var(--spacing-lg)',
            fontSize: 'var(--font-size-sm)',
            color: 'var(--color-gray-600)'
          }}>
            ログインすると、本サービスの
            <a
              href="/privacy"
              style={{ color: 'var(--color-primary)', textDecoration: 'underline' }}
            >
              プライバシーポリシー
            </a>
            に同意したものとみなされます。
          </p>
        )}
      </div>
    </div>
  );
}
